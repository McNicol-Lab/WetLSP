...
# divide images into chunks, and save them as temporal files
...
foreach(i=1:length(dates)) %dopar%{
  ...
  band5 <- values(raster(files[i],5))
  band6 <- values(raster(files[i],6))
  band7 <- values(raster(files[i],7))
  band8 <- values(raster(files[i],8))
  ...
  foreach(cc=1:numCk) %dopar%{
    ...
    save(b1,b2,b3,b4,b5,b6,b7,b8,file=paste0(dirTemp,'/',yy[i],mm[i],dd[i],'.rda'))
  }
}
...
foreach(cc=1:numCk) %dopar%{
  ...
  if(length(files)==length(dates)){
    ...
    band5 <- matrix(NA,length(chunks),length(dates))
    band6 <- matrix(NA,length(chunks),length(dates))
    band7 <- matrix(NA,length(chunks),length(dates))
    band8 <- matrix(NA,length(chunks),length(dates))
    ...
    # Save
    save(band1,band2,band3,band4,band5,band6,band7,band8,dates,
