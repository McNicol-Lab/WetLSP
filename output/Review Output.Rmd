...
```{r view-data-01}
file_path <- ("FI-Si2/mosaic")
file_names <- list.files(file_path)
terra::rast(paste(file_path, file_names[20], sep = "/")) %>% 
  plot()

```

```{r alternative}
r <- rast("FI-Lom/mosaic/20210605_clipped_mosaic.tif")  # pick a date you know is good
r

summary(values(r[[8]]))   # band 
NAflag(r)
unique(values(r[[1]])[values(r[[1]]) > 1e9])
```
...
```{r view-chunks}
e <- new.env()
load("DE-Hte/chunk/chunk_001.rda", envir = e)
e$band6[,201] %>% summary()
```
...
```{r chunk-sf}
readRDS("FI-Si2/tables_sf/chunk_001_evi_sf.rds")[1:3,]
```

Let's also look at the .csv version

```{r chunk-csv}
sf_obj <- readRDS("BW-Gum/tables_sf/chunk_001_evi_sf.rds")

# how many pixels have any spline series at all?
sum(lengths(sf_obj$evi_spline) > 0)

# proportion of non-NA values across all splines
...
```{r}
load("FI-Si2/chunk_phe/chunk_phe_001.rda")
view(pheno_mat)
rm(pheno_mat)
```
...
list.files("Product_GeoTiff/BW-Gum/2021", "\\.tif$", full.names = TRUE)[10] %>%
  rast() %>%
  (\(x) plot(x/10000,
             main = "BW-Gum – EVI Max - 2022",
             axes = TRUE,
             mar = c(4,4,2,3)))()
```

```{r}
r <- rast("Product_GeoTiff/CA-EM1/2021/09_2021_EVImax.tif")

# Check the actual dimensions in meters
...
r_trim <- trim(r)
plot(r_trim, main = "BR-Npw – Num Cycles", axes = TRUE)
```
...
```{r use-helper}
sf_all <- readRDS("/tables_site/AT-Nsd_evi_timeseries_sf.rds")
...
```{r use-mapper}
sf_chunk <- readRDS("KR-CRK/tables_sf/chunk_100_evi_sf.rds")
PickPixelInteractive(sf_chunk)
ViewPixelTimeseries(sf_chunk, cell_vec = c(seq(677249, 678249, by = 100)))
```


## Debugging - site 23

✅ 20241231 → 200 chunks written
Error in { : task 127 failed - "cannot open the connection"

```{r}
library(terra)

mosaic_dir <- "/Users/gawainmacneacail/Documents/Research/Work/PlanetScope/WetLSP/output/KR-CRK/mosaic"   # adjust to your actual site name
mosaic_files <- list.files(
  mosaic_dir,
  pattern = "_clipped_mosaic\\.tif$",
  full.names = TRUE
)

length(mosaic_files)      # just to confirm
mosaic_files[395]         # this is the one that blew up
```
...
```{r}
tmpChunkDir <- file.path("/Users/gawainmacneacail/Documents/Research/Work/PlanetScope/WetLSP/output/IT-Cas/chunk", "temp")
tmpChunkDir

# dir.exists(tmpChunkDir)

file.access(tmpChunkDir, 2)  # 0 = writable
```
